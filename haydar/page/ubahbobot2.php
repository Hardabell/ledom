...
$querylihat="SELECT id_lomba,bobot,id_bobotkriteria,kriteria.nama_kriteria AS namaKriteria FROM bobot_kriteria INNER JOIN kriteria USING(id_kriteria) WHERE id_lomba='$id'";
...
<form id="form" action="./proses/prosesubah.php" method="POST">
    ...
    <div class="panel-middle">
        <div class="group-input">
            <div class="group-input">
                <?php
                $query="SELECT nama_lomba FROM lomba WHERE id_lomba='$id'";
                $execute=$konek->query($query);
                $data=$execute->fetch_array(MYSQLI_ASSOC);
                ?>
                <div class="group-input">
                    <label for="jenisbarang">Jenis Lomba</label>
                    <input class="form-custom" value="<?php echo $data['nama_lomba'];?>" disabled type="text" autocomplete="off" required name="barang" id="barang">
                </div>
            </div>
        </div>
        <?php
